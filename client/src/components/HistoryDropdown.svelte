<script lang="ts">
  ...
  export let history: Array<{question: string, answer: string | null, query: string | null, timestamp: Date}> = [];
  export let onClose: () => void = () => {};
  export let onSelect: (historyItem: {question: string, answer: string | null, query: string | null, timestamp: Date}) => void = () => {};
...
{#if isVisible}
  ...
  <!-- Dropdown -->
  <div class="fixed z-50 history-dropdown" style="top: {position.top}px; left: {position.left}px;">
    <div class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl max-w-md w-80 max-h-96 overflow-hidden">
      <!-- Header -->
      <div class="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700">
        <h3 class="text-sm font-medium text-gray-900 dark:text-gray-100">AI Conversation History</h3>
        <button 
          on:click={onClose}
          ...
          {#each history as historyItem, index}
            ...
            >
              <div class="flex items-start justify-between mb-2">
                <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100 line-clamp-2 flex-1">
