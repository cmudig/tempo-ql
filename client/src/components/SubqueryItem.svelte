...
<div
  class="border border-gray-300 dark:border-gray-600 rounded-md mb-2 overflow-hidden"
>
    ...
    {#if data.dataset_queries && data.dataset_queries.length > 0}
      <div class="p-4 border-t border-gray-200 dark:border-gray-700">
        ...
        {#each data.dataset_queries as sql}
          <pre
            class="bg-white dark:bg-gray-900 text-xs font-mono p-2 rounded">{sql}</pre>
        {/each}
      </div>
    {/if}
  {/if}
