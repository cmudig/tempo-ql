<script lang="ts">
  ...
  function formatSQL(sql: string): string {
    ...
    let formatted = sql
      ...
      .replace(/\b(SELECT|FROM|WHERE|JOIN|LEFT JOIN|RIGHT JOIN|INNER JOIN|GROUP BY|ORDER BY|HAVING|UNION|CASE|WHEN|THEN|ELSE|END)\b/gi, '\n$1')
      // Add line breaks after commas in SELECT clauses
      .replace(/(SELECT\s+)(.*?)(\s+FROM)/gis, (match, select, fields, from) => {
      ...
      .map((line, index) => {
        ...
        let indent = '';
        if (trimmed.match(/^(SELECT|FROM|WHERE|JOIN|GROUP BY|ORDER BY|HAVING|UNION|CASE|WHEN|THEN|ELSE|END)/i)) {
          indent = '';
...
<div class="bg-white dark:bg-gray-900 p-4 border-0 rounded-none {width} transition-colors duration-200">
  <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">Query Output</h3>
  
  {#if isSQL}
    <!-- SQL Query Display -->
    <div class="bg-gray-100 dark:bg-gray-800 rounded-md p-4 border border-gray-300 dark:border-gray-600">
      <div class="flex items-center justify-between mb-3">
        <span class="text-blue-600 dark:text-blue-400 text-sm font-mono font-semibold">SQL Query</span>
        <button
      ...
      <pre class="text-green-600 dark:text-green-400 text-sm leading-relaxed font-mono overflow-x-auto whitespace-pre-wrap">{formattedSQL}</pre>
    </div>
  {:else}
    <!-- Regular Message Display -->
    <div class="bg-gray-100 dark:bg-gray-800 rounded-md p-4 border border-gray-300 dark:border-gray-600">
      <p class="text-gray-700 dark:text-gray-200 text-sm leading-relaxed font-mono">
