...
<div
  class="bg-white dark:bg-gray-900 p-4 mb-0 border-0 rounded-none {width} transition-colors duration-200 {scrollable
...
>
  ...
  <div class="flex items-center justify-between mb-2">
    ...
    <svg
      ...
      viewBox="0 0 24 24"
      class="text-gray-400"
      ><path d="M15 8a3 3 0 1 0-6 0v8a3 3 0 1 0 6 0V8z" /><path
        d="M12 3v2m0 14v2m7-7h2m-18 0H3"
      /></svg
    >
  </div>
    ...
    <div class="space-y-2">
      ...
      >
        <div class="flex items-center">
          ...
          <span class="font-bold text-gray-900 dark:text-gray-100 mr-2"
            >Count:</span
          >
        ...
        {#if showCountDetails}
          <div
            class="mt-2 p-2 bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded text-sm text-gray-700 dark:text-gray-300"
          >
            <div class="mb-2">
              <div
                class="flex flex-wrap justify-between items-center mb-1 gap-1"
              >
                <span class="text-gray-600 dark:text-gray-400">Unique IDs:</span
                >
                <span
                  class="font-mono font-semibold text-gray-700 dark:text-gray-200 text-xs sm:text-sm"
                >
                  {formatNumber(values.values.num_unique_ids ?? 0)} / {formatNumber(
                    idsLength
                  )}
                </span>
              </div>
              <SliceMetricBar
                value={idsLength > 0
                  ? (values.values.num_unique_ids ?? 0) / idsLength
                  : 0}
                width={null}
                showFullBar
              >
            ...
            <div>
              <div class="flex justify-between items-center mb-1">
                <span class="text-gray-600 dark:text-gray-400"
                  >Number of timesteps per trajectory:</span
                >
              </div>
      ...
      >
        <div class="flex items-center">
          ...
          >
          <span class="tabular-nums text-gray-700 dark:text-gray-200"
            >{d3.format('.1%')(values.values.missingness ?? 0)}</span
          >
        </div>
        {#if showMissingnessDetails}
          <div
          ...
          >
            <div>
              ...
              >
                <span class="text-gray-600 dark:text-gray-400"
                  >Number of missing values:</span
                >
                <span
                  class="font-mono font-semibold text-gray-700 dark:text-gray-200 text-xs sm:text-sm"
                >
                  {formatNumber(values.values.num_missing_values ?? 0)} / {formatNumber(
              ...
              >
                <span slot="caption">
                  <strong
                    >{d3.format('.1%')(values.values.missingness ?? 0)}</strong
                  >
                  <span class="text-gray-500 dark:text-gray-400 ml-1 text-xs">
                    ({formatNumber(values.values.num_missing_values ?? 0)} / {formatNumber(
                      values.values.length ?? 0
              ...
              {#if values.values.missing_ids_hist && Object.keys(values.values.missing_ids_hist).length > 0}
                <div class="flex justify-between items-center mb-1">
                  <span class="text-gray-600 dark:text-gray-400"
                    >Number of missing values per trajectory:</span
                  >
                </div>
                <SliceMetricHistogram
        ...
        {#if values.values.type == 'binary'}
          ...
          >
            <span slot="caption">
              ...
              true,
              <strong>{d3.format('.1%')(1 - (values.values.mean ?? 0))}</strong>
              false
            </span>
          </SliceMetricBar>
        ...
        {:else if values.values.type == 'categorical'}
          <SliceMetricCategoryBar
            order={Object.keys(values.values.counts || {}).sort(
              (a, b) =>
                (values.values.counts?.[b] ?? 0) -
                (values.values.counts?.[a] ?? 0)
            )}
