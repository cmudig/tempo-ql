<script lang="ts">
  ...
  const { message, values, idsLength, listNames, handleTextInput, handleLLMQuestion, subqueryEnabled, subqueries, scopes, selectedScope, scopeConcepts, handleScopeAnalysis, llmMessage, llmLoading, llmError, apiStatus, isLoading, loadingMessage, extractedQuery, aiExplanation, hasExtractedQuery, handleQueryExtraction, textInputStore } = backend;
  ...
  $: {
    if ($textInputStore && $textInputStore !== textInput) {
      console.log('🔄 Frontend: Syncing text input from backend:', $textInputStore);
  ...
  let aiHistory: Array<{question: string, answer: string | null, query: string | null, timestamp: Date}> = [
  ...
  $: if ($llmMessage && $llmMessage.trim() && !$llmLoading && currentQuestion && !historicalResponse) {
  ...
  function handleExplain() {
    if (textInput.trim()) {
      ...
      handleLLMQuestion(`Please explain this TempoQL query: ${textInput}`, 'explain');
    }
  }
  
  // Add new conversation to history
  function addToHistory(question: string, answer: string | null, query: string | null = null) {
    const newEntry = {
      question,
      answer,
      query,
  ...
  function calculateHistoryDropdownPosition() {
    ...
    if (historyButton) {
      ...
      (historyButton as HTMLElement).style.backgroundColor = 'rgba(255, 0, 0, 0.2)';
      setTimeout(() => {
      ...
      while (parent && parentPath.length < 5) {
        parentPath.push(parent.tagName + (parent.className ? '.' + parent.className.split(' ')[0] : ''));
      ...
      if (historyDropdownPosition.left + dropdownWidth > window.innerWidth - 10) {
  ...
  function calculateQueryHistoryDropdownPosition() {
    ...
    if (queryHistoryButton) {
      ...
      console.log('📍 Calculated query dropdown position:', queryHistoryDropdownPosition);
      ...
      // Ensure dropdown doesn't go off-screen to the right
      if (queryHistoryDropdownPosition.left + dropdownWidth > window.innerWidth - 10) {
        queryHistoryDropdownPosition.left = window.innerWidth - dropdownWidth - 10;
      }
      ...
      console.log('📍 Final query dropdown position:', queryHistoryDropdownPosition);
    } else {
  ...
  function handleHistorySelect(historyItem: {question: string, answer: string | null, query: string | null, timestamp: Date}) {
  ...
  function handleQueryHistorySelect(historyItem: {question: string, answer: string | null, query: string | null, timestamp: Date}) {
...
<main class="w-full max-w-full mx-auto bg-white dark:bg-gray-950 h-screen transition-colors duration-200 relative overflow-hidden">
  <!-- Loading Bar -->
  ...
  <div class="p-4 h-[calc(100vh-4rem)] overflow-hidden">
    ...
    {:else if activeTab === 'query-inspector'}
      <QueryInspectorTab 
        ...
        aiInputValueOverride={aiInputValueOverride}
