...
export function getAutocompleteOptions(
  ...
  console.log('🔍 getAutocompleteOptions - Input:', {
    dataFields,
    searchQuery,
    fullPrefix,
  });
  ...
  if (!!result) {
    ...
    const allMatches = [...exactMatches, ...partialMatches].map((v) => ({
      value: v,
      type: 'data_item',
    }));
...
export function performAutocomplete(
  ...
  console.log('🔍 performAutocomplete - Input:', {
    item,
    trigger,
    fullPrefix,
    fullSuffix,
  ...
  if (item.type == 'data_item') {
    ...
    if (!!result) {
      ...
      console.log(
        '🔍 performAutocomplete - Data item replacement (with existing):',
        replacement
      );
      return replacement;
    }
    ...
    console.log(
      '🔍 performAutocomplete - Data item replacement (new):',
      replacement
    );
    return replacement;
  }
